...
//             }
        ...
        bg = new FlxSprite().loadGraphic(Paths.image('coffeeshop2', 'preload'));
        else
        bg = new FlxSprite().loadGraphic(Paths.image('coffeeshop1', 'preload'));
        ...
        charBop = new FlxSprite(400, -200);
		charBop.frames = Paths.getSparrowAtlas('characters/origamigf', 'shared');
		charBop.animation.addByPrefix('idle', 'GF Dancing Beat', 24, false);
        charBop.setGraphicSize(Std.int(charBop.width * 0.7));
		charBop.antialiasing = true;
        charBop.flipX = false;
		add(charBop);

        var gamejoltlogo:FlxSprite = new FlxSprite(-50, 40).loadGraphic(Paths.image('gamejolt', 'preload'));
		gamejoltlogo.antialiasing = true;
        gamejoltlogo.setGraphicSize(Std.int(gamejoltlogo.width * 0.5));
		add(gamejoltlogo);

        var xtxt:FlxText = new FlxText(450, 100, "x" , 60);
        add(xtxt);

        achivementtxt = new FlxText(150, 230, "Sign in to unlock achivements!" , 20);
            achivementtxt.visible = false;
            add(achivementtxt);

        var logo:FlxSprite = new FlxSprite(200, -250).loadGraphic(Paths.image('TheOrigamiKingLogo'));
        ...
        usernameText = new FlxText(0, 325, 300, "Username:", 30);

        tokenText = new FlxText(0, 425, 300, "Token:", 30);
        ...
        usernameBox = new FlxUIInputText(0, 375, 300, null, 32, FlxColor.BLACK, FlxColor.GRAY);
        tokenBox = new FlxUIInputText(0, 475, 300, null, 32, FlxColor.BLACK, FlxColor.GRAY);
        ...
        signInBox = new FlxButton(250, 550, "Login", function()
        {
        ...
        helpBox = new FlxButton(580, 485, "Find Your Token", function()
        {
            openLink('https://www.youtube.com/watch?v=T5-x7kAGGnE');
        });
        helpBox.color = FlxColor.fromRGB(186,85,211);

        logOutBox = new FlxButton(250, 550, "Log Out & Restart", function()
        {
            GameJoltAPI.deAuthDaUser();
        });
        #if !windows
        logOutBox.text = "Log Out";
        #end
        logOutBox.color = FlxColor.RED /*FlxColor.fromRGB(255,134,61)*/ ;

        cancelBox = new FlxButton(500, 550, "Not Right Now", function()
        {
            FlxG.sound.play(Paths.sound('confirmMenu'), 0.7, false, null, true, function(){
                GameJoltAPI.getTrophy(150041);
                FlxG.switchState(new MainMenuState());
            });
        });

        if(!GameJoltAPI.getStatus())
        achivementtxt.visible = true;
        else
        achivementtxt.visible = false;
        ...
        if(GameJoltAPI.getStatus())
        {
            username = new FlxText(0, 300, 0, "Signed in as:\n" + GameJoltAPI.getUserInfo(true), 40);
